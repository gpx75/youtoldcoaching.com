---
title: Guiding leaders from uncertainty to empowered leadership.
bodyHeading: ""
contactForm: false
cta:
  label: View programmes
  href: /services
cta2:
  label: Complimentary 30-min call
  href: /contact?type=30
description: Moya James — ICF Level 2 certified leadership coach with 20+ years in pharmaceutical industry leadership. Creator of the Pathwise™ methodology. Based in the UK and Switzerland, coaching globally.
eyebrow: About Moya James
heroBgImage: /images/about-hero.jpg
heroContentScroll: false
heroIllustrationFill: false
sections:
  - type: differentiators
    eyebrow: What makes this approach different
    heading: Deep expertise. Proprietary methodology. Bespoke outcomes.
    pillars:
      - label: Corporate sector depth
        body: 20+ years leading in pharmaceutical, banking, and technology — the sectors where most of my clients come from. I understand the culture, the politics, and the identity costs of leaving.
      - label: Global and cross-cultural reach
        body: Based across the UK and Switzerland with clients in emerging markets. Leadership transitions are shaped by context. Cultural fluency is built into the work.
      - label: Pathwise™ Methodology
        body: A proprietary framework — Activate → Align → Achieve — developed across hundreds of engagements. Not adapted from a generic model. Built from the transitions themselves.
      - label: Bespoke, high-touch programming
        body: No cohorts. No group sessions. Every programme is designed around your specific transition, timeline, and leadership identity.
      - label: Story-driven, journey-focused
        body: Narrative coaching at the core. The story you tell about your transition shapes the leader you become on the other side of it. This is the foundational conviction.
      - label: Internal and external impact
        body: The work addresses both inner leadership development — clarity, capability, confidence — and external executive presence, stakeholder influence, and career trajectory.
  - type: credibility
    eyebrow: Background and credentials
    stats:
      - value: 20+
        label: Years pharmaceutical leadership
      - value: "2016"
        label: Founded coaching practice
      - value: ICF L2
        label: Certified coach
      - value: 400+
        label: Leaders coached
  - type: action
    eyebrow: Work with Moya
    heading: Ready to begin? Let's find out if this is the right fit.
    body: Start with a complimentary 30-minute conversation. No obligation, no pitch — a real discussion about where you are and what your transition requires.
    cta:
      label: Book Alignment Session — £120
      href: /contact?type=90
    cta2:
      label: Complimentary 30-min call
      href: /contact?type=30
subtitle: I know what it feels like to leave behind a version of yourself that everyone else thought was working fine.
subtitleFont: script
tags:
  - 20+ years pharmaceutical leadership
  - ICF Level 2 certified
  - Pathwise™ creator
---

I help senior leaders and entrepreneurs navigate pivotal career transitions through the Pathwise™ framework. I guide clients from uncertainty to empowered leadership, enabling them to step into their next chapter with authority and purpose.

After more than 20 years leading teams across global pharmaceutical organisations — spanning the UK, Switzerland, and emerging markets — I made a deliberate choice to leave corporate life. Not because something was wrong. Because something else was calling.

In 2016 I founded my coaching practice, built on a conviction I had tested in boardrooms, in transitions, and in my own professional evolution: **the story you tell about your transition shapes the leader you become on the other side of it.**

That conviction became the Pathwise™ methodology. It is narrative-centred, evidence-informed, and designed specifically for the kind of pivotal transitions that senior leaders face — not entry-level career changes, but identity-level reinventions.

My industry background spans **Pharmaceutical · Banking · Technology**. My clients come from those sectors because I understand them from the inside.

So today I change very little test
